...
export class AccountRepository implements IAccountRepository {
    ...
    async getAccountByClientId(client_id: string): Promise<Account | null> {
        const [accountData] = await this.connection("account").where("client_id", client_id);

        if (!accountData) return null;

        return Account.create(accountData.client_id, accountData.name, accountData.sequence, accountData.created_at);
    }
